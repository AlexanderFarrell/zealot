...
export class BasicAPI<Type, Dto, AddDto, UpdateDto> {
    protected URL: string;
    protected Factory: (dto: Dto) => Type;

    constructor(url: string, factory: (dto: Dto) => Type) {
        this.URL = url;
        this.Factory = factory;
    }

    async get_by_id(id: number): Promise<Type | null> {
        let dto = (await get_json(`${this.URL}/id/${id}`)) as Dto | null
        if (dto == null) {
            return null;
        }
    ...
    async get_all(): Promise<Type[]> {
        let dtos = (await get_json(this.URL)) as Dto[]
        return dtos.map(d => {
...
export class LazyData<T> {
    private data_in_flight: Promise<T> | null = null;
    private data: T | null = null;
    private source: () => Promise<T>;
    private dirty: boolean = false;

    public constructor(source: () => Promise<T>) {
        this.source = source;
    }
    ...
    public async Get() {
        ...
        if (this.data_in_flight) {
            return this.data_in_flight;
        }

        this.data_in_flight = this.source().then(d => {
            this.data = d;
            this.data_in_flight = null;
            return this.data;
        }).catch(err => {
            this.data_in_flight = null;
            this.data = null;
