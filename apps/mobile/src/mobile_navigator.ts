...
export class MobileNavigator implements Navigator {
    ...
    openAnalysisRecent(): void {
        this.navigate('/analysis/recent');
    }

    openAnalysisBacklog(): void {
        this.navigate('/analysis/backlog');
    }
    ...
    private matchRoute(path: string): RouteMatch {
        if (path === '/') {
            ...
            s.loadItem('Home');
            return { screen: s, location: { kind: 'home' } };
        }
        if (path === '/analysis') {
            return { screen: new AnalysisScreen(), location: { kind: 'analysis' } };
        }
        if (path === '/analysis/specify') {
            return { screen: new SpecifyScreen(), location: { kind: 'analysis_specify' } };
        }
        if (path === '/analysis/working') {
            return { screen: new WorkingScreen(), location: { kind: 'analysis_working' } };
        }
        if (path === '/analysis/recent') {
            return { screen: new RecentScreen(), location: { kind: 'analysis_recent' } };
        }
        if (path === '/analysis/backlog') {
            return { screen: new BacklogScreen(), location: { kind: 'analysis_backlog' } };
        }
        ...
        if (path === '/types') {
            return { screen: new TypesScreen(), location: { kind: 'types' } };
        }
        ...
        if (segments[0] === 'item' && segments[1]) {
            ...
            const s = new ItemScreen();
            s.loadItem(title);
            return { screen: s, location: { kind: 'item', title } };
        }
        ...
        if (segments[0] === 'item_id' && segments[1]) {
            ...
            const s = new ItemScreen();
            s.loadItemById(id);
            return { screen: s, location: { kind: 'item', itemId: id } };
        }

        if (segments[0] === 'media') {
            const mediaPath = path === '/media' ? '' : path.replace(/^\/media\//, '');
            return { screen: new MediaScreen().init(mediaPath), location: { kind: 'media', path: mediaPath } };
        }
        ...
        if (segments[0] === 'planner') {
            ...
            if (view === 'daily') {
                if (!date) return this.redirect(`/planner/daily/${todayIso()}`);
                return { screen: new DailyPlannerScreen().init(date), location: { kind: 'planner', view, date } };
            }
            if (view === 'weekly') {
                if (!date) return this.redirect(`/planner/weekly/${thisWeekIso()}`);
                return { screen: new WeeklyPlannerScreen().init(date), location: { kind: 'planner', view, date } };
            }
            if (view === 'monthly') {
                if (!date) return this.redirect(`/planner/monthly/${thisMonthIso()}`);
                return { screen: new MonthlyPlannerScreen().init(date), location: { kind: 'planner', view, date } };
            }
            if (view === 'annual') {
                if (!date) return this.redirect(`/planner/annual/${thisYear()}`);
                return { screen: new AnnualPlannerScreen().init(date), location: { kind: 'planner', view, date } };
            }
        }
        ...
        if (segments[0] === 'settings') {
            ...
            return { screen: s, location: { kind: 'settings', section } };
        }
    ...
    private redirect(path: string): RouteMatch {
        return { screen: document.createElement('div'), location: this.location, redirectTo: path };
    }

    private notFound(path: string): RouteMatch {
        const div = document.createElement('div');
        div.textContent = `404 Not Found: ${path}`;
        return { screen: div, location: { kind: 'not_found', path } };
    }
}
