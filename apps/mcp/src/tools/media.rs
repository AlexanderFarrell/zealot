...
impl ZealotServer {
    #[rmcp::tool(
        description = "List the contents of the media file system. Pass an empty path for the root, or a relative path for a subdirectory. Returns file/folder metadata."
    )]
    ...
    #[rmcp::tool(
        description = "Create a new folder in the media file system. The folder path is relative (e.g. 'images/2025')."
    )]
    ...
    #[rmcp::tool(
        description = "Delete a file or directory from the media file system. The path is relative. Directories must be empty before deletion."
    )]
