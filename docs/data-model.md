...
Scopes are persistent ownership boundaries with active memberships. The only v1 roles are `owner`, `editor`, and `viewer`; roles are the same for human and service principals, and custom permissions are not supported. Existing clients without an explicit scope selection continue to use the authenticated principal's default scope through the Z139 compatibility path. Scope invitations are opaque, scope-bound, expiring, single-use capabilities: only a secure token hash and an HMAC signature made with a server-local signing key are stored. An invitation is never a login session, API key, password reset, or replicated credential. Membership and invitation transitions emit redacted scope lifecycle events with actor, outcome, reason code, and correlation ID.

An active scope member may be listed only by an authorized scope reader; invitation creation, approval, cancellation, revocation, role changes, and member removal require the `manage_members` grant (the v1 owner preset). A scope cannot lose its final active owner. New-account invitation acceptance remains pending until the host server's enrolment policy and Z180 account lifecycle complete; invitation acceptance does not authenticate the bearer. Replicated external subjects require an explicit verified local mapping; display name or email matching is not sufficient.
