...
use zealot_domain::{
    ...
    item::{AddItemDto, Item, SearchScope, UpdateItemDto},
};
        ...
        items_table.set(
            ...
            lua.create_async_function(move |lua, term: String| {
                ...
                async move {
                    let items = svc
                        .item
                        .search_items_in_scopes(&term, SearchScope::Title, false, 20, 0, &access)
                        .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?
                        .into_iter()
        ...
        items_table.set(
            ...
            lua.create_async_function(move |lua, filters_table: Table| {
                ...
                async move {
                    ...
                    let items = svc
                        .item
                        .filter_items_in_scopes(&filter_dtos, &access)
                        .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
                    items_vec_to_lua(&lua, &items)
                }
            })?,
            ...
            lua.create_async_function(
                move |lua, (title, content, opts): (String, Option<String>, Option<Table>)| {
                    ...
                    async move {
                        ...
                        match svc
                            .item
                            .add_item_in_scope(&dto, &access, &account)
                            .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?
                        {
                            Some(item) => Ok(LuaValue::Table(item_to_lua(&lua, &item)?)),
                            None => Ok(LuaValue::Nil),
        ...
        items_table.set(
            ...
            lua.create_async_function(move |lua, (id, opts): (i64, Table)| {
                ...
                async move {
                    ...
                    match svc
                        .item
                        .update_item_in_scopes(&item_id, &dto, &access)
                        .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?
                    {
                        Some(item) => Ok(LuaValue::Table(item_to_lua(&lua, &item)?)),
                        None => Ok(LuaValue::Nil),
        ...
        items_table.set(
            ...
            lua.create_async_function(move |_, (id, key, value): (i64, String, LuaValue)| {
                ...
                async move {
                    ...
                    svc.item
                        .set_attributes_in_scopes(&item_id, &map, &access)
                        .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
                    Ok(true)
                }
            })?,
            ...
            lua.create_async_function(move |_, (id, type_name): (i64, String)| {
                ...
                async move {
                    let item_id =
                        Id::try_from(id).map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
                    svc.item
                        .assign_type_in_scopes(&type_name, &item_id, &access)
                        .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
                    Ok(true)
                }
            })?,
            ...
            lua.create_async_function(move |_, id: i64| {
                ...
                async move {
                    let item_id =
                        Id::try_from(id).map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
                    svc.item
                        .delete_item_in_scopes(&item_id, &access)
                        .map_err(|e| mlua::Error::RuntimeError(e.to_string()))?;
                    Ok(true)
                }
            })?,
