...
fn row_to_attribute_kind(row: AttributeKindRow) -> Result<AttributeKind, RepoError> {
    ...
    let kind_id = Id::try_from(row.kind_id as i64)
...
impl AttributeRepo for AttributePostgresRepo {
            ...
            tokio::runtime::Handle::current().block_on(async move {
                sqlx::query_as::<_, AttributeKindRow>(
                    "SELECT kind_id, account_id, key, description, base_type, config, is_system
                     FROM attribute_kind
                     WHERE key = $1 AND (account_id = $2 OR is_system = true)",
                )
                .bind(&key)
                .bind(account_id_val)
                .fetch_optional(&pool)
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                sqlx::query_as::<_, AttributeKindRow>(
                    "SELECT kind_id, account_id, key, description, base_type, config, is_system
                     FROM attribute_kind
                     WHERE kind_id = $1 AND (account_id = $2 OR is_system = true)",
                )
                .bind(id_val)
                .bind(account_id_val)
                .fetch_optional(&pool)
    ...
    fn update_attribute_kind(
        ...
        let config = dto
            .config
            .as_ref()
            .map(|c| serde_json::to_value(c).unwrap_or(Value::Object(Default::default())));
        let pool = self.pool.clone();
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                sqlx::query_as::<_, AttributeKindRow>(
                    "UPDATE attribute_kind
                     SET key = COALESCE($1, key),
                         ...
                         base_type = COALESCE($3, base_type),
                         config = COALESCE($4::jsonb, config)
                     WHERE kind_id = $5 AND account_id = $6
                     RETURNING kind_id, account_id, key, description, base_type, config, is_system",
                )
                .bind(key)
                .bind(description)
    ...
    fn delete_attribute_kind(&self, key: &str, account_id: &Id) -> Result<(), RepoError> {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                sqlx::query("DELETE FROM attribute_kind WHERE key = $1 AND account_id = $2")
                    .bind(&key)
                    .bind(account_id_val)
                    .execute(&pool)
                    .await
