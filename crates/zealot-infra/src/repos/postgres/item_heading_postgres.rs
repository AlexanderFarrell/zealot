...
#[derive(Debug)]
pub struct ItemHeadingPostgresRepo {
    pool: PgPool,
}

impl ItemHeadingPostgresRepo {
...
impl ItemHeadingRepo for ItemHeadingPostgresRepo {
    ...
    fn replace_for_item(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                ...
                sqlx::query("DELETE FROM item_heading WHERE item_id = $1")
                    .bind(item_id_val)
                    .execute(&mut *tx)
                    .await
                    .map_err(RepoError::from)?;
                ...
                for (level, ordinal, text) in &headings {
                    sqlx::query(
                        "INSERT INTO item_heading (item_id, level, ordinal, text) VALUES ($1, $2, $3, $4)",
                    )
                    .bind(item_id_val)
                    .bind(*level as i32)
                    .bind(*ordinal as i32)
