...
impl ItemLinkRepo for ItemLinkPostgresRepo {
    fn get_source_item_ids_in_scopes(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                let rows = sqlx::query_as::<_, ScopedItemIdRow>(
                    "SELECT src.item_id, src.account_id
                     ...
                     WHERE target.item_id = $1
                       AND target.scope_id = ANY($2)
                       AND src.scope_id = ANY($2)
                ...
                rows.into_iter()
                    .map(|row| {
                        Ok((
                            Id::try_from(row.item_id as i64).map_err(|err| {
                                RepoError::DatabaseError {
                                    err: err.to_string(),
                                }
                            })?,
                            Id::try_from(row.account_id as i64).map_err(|err| {
                                RepoError::DatabaseError {
                                    err: err.to_string(),
                                }
                            })?,
    ...
    fn get_related_item_ids_in_scopes(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                let outgoing = sqlx::query_as::<_, ScopedItemIdRow>(
                    "SELECT target.item_id, target.account_id
                     FROM item_item_link l
                     JOIN item src ON src.item_id = l.first_item_id
                     JOIN item target ON target.item_id = l.second_item_id
                     WHERE l.first_item_id = $1
                       AND src.scope_id = ANY($2)
                       AND target.scope_id = ANY($2)
                ...
                .await
                .map_err(RepoError::from)?;
                let incoming = sqlx::query_as::<_, ScopedItemIdRow>(
                    "SELECT src.item_id, src.account_id
                     FROM item_item_link l
                     JOIN item src ON src.item_id = l.first_item_id
                     JOIN item target ON target.item_id = l.second_item_id
                     WHERE l.second_item_id = $1
                       AND src.scope_id = ANY($2)
                       AND target.scope_id = ANY($2)
                       AND l.relationship != 'parent'
                     ORDER BY src.item_id",
    ...
    fn get_links_for_items_in_scopes(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                ...
                for row in rows {
                    let first = Id::try_from(row.first_item_id as i64)
                        .map_err(|e| RepoError::DatabaseError { err: e.to_string() })?;
                    let second = Id::try_from(row.second_item_id as i64)
                        .map_err(|e| RepoError::DatabaseError { err: e.to_string() })?;
                    links_by_item
                        .entry(first)
