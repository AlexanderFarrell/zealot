    ...
    fn get_for_day_scoped(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                ...
                let item_ids: Vec<i32> = item_rows.iter().map(|row| row.item_id).collect();
                let entry_rows = sqlx::query_as::<_, RepeatEntryRow>(
                    "SELECT re.item_id, re.status::text, re.comment
                     FROM repeat_entry re
                     JOIN item i ON i.item_id = re.item_id
                     WHERE re.date = $1
                       AND i.scope_id = ANY($2)
                       AND re.item_id = ANY($3)",
                )
                .bind(day)
                .bind(&scope_ids)
                .bind(&item_ids)
                ...
                let mut entry_map: HashMap<i64, RepeatEntryRow> = entry_rows
                    .into_iter()
                    .map(|row| (row.item_id as i64, row))
                    .collect();
    ...
    fn get_for_range_scoped(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async move {
                ...
                let item_schedule: Vec<(i32, String, Option<NaiveDate>)> = item_rows
                    .iter()
                    .map(|row| (row.item_id, row.schedule.clone(), row.end_date))
                    .collect();
                let item_ids: Vec<i32> = item_rows.iter().map(|row| row.item_id).collect();
                let entry_rows = sqlx::query_as::<_, RepeatEntryWithDateRow>(
                    "SELECT re.item_id, re.date, re.status::text, re.comment
                     FROM repeat_entry re
                     JOIN item i ON i.item_id = re.item_id
                     WHERE re.date BETWEEN $1 AND $2
                       AND i.scope_id = ANY($3)
                       AND re.item_id = ANY($4)",
                )
                .bind(start)
                .bind(end)
                .bind(&scope_ids)
                ...
                let mut entry_map: HashMap<(i64, NaiveDate), RepeatEntryWithDateRow> = entry_rows
                    .into_iter()
                    .map(|row| ((row.item_id as i64, row.date), row))
                    .collect();
                ...
                loop {
                    ...
                    for (item_id_val, schedule, item_end_date) in &item_schedule {
                        ...
                        let item_id = Id::try_from(*item_id_val as i64)
                            .map_err(|e| RepoError::DatabaseError { err: e.to_string() })?;
                        let key = (*item_id_val as i64, current);
