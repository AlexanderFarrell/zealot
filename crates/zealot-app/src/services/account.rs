...
impl AccountService {
    ...
    pub fn generate_api_key(&self, account_id: &Id, label: &str) -> Result<(ApiKeyRecord, String), ServiceError<AccountError>> {
    ...
    pub fn list_api_keys(&self, account_id: &Id) -> Result<Vec<ApiKeyRecord>, ServiceError<AccountError>> {
        self.repo
            .list_api_keys(account_id)
            .map_err(|_| ServiceError::DomainError { err: AccountError::ServerError })
    }

    pub fn revoke_api_key(&self, api_key_id: &Id, account_id: &Id) -> Result<(), ServiceError<AccountError>> {
        self.repo
